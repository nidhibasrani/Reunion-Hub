...
router.post(
  "/add-event",
  auth,
  eventImage("featuredImage", "gallery"),
  async (req, res) => {
    const { role } = req.user;
    console.log(role);
    if (role !== "admin") {
  ...
  async (req, res) => {
    try {
      ...
      if (!req.files || req.files.length === 0) {
        return res
          .status(400)
          .json({ message: "No images uploaded or invalid image format" });
      }

      // Extract the paths of the uploaded images
      const galleryPaths = req.files.map((file) => file.path);

      // Find the Cms document or create a new one if not found
      let cmsDocument = await Cms.findOne();
      if (!cmsDocument) {
        cmsDocument = new Cms();
      }

      // Push new gallery paths into the document's gallery array
      cmsDocument.gallery.push(...galleryPaths);
...
router.get("/website-gallery", auth, async (req, res) => {
  try {
    const { role } = req.user;
    if (role !== "admin") {
      return res.status(401).json({ msg: "Unauthorized" });
